...
const Step1 = ({ onNext }) => {
  ...
  const correctSecret = "mansi"; // Keep this as your secret
  ...
  const checkSecret = () => {
    if (input.toLowerCase().trim() === correctSecret) {
      ...
      setIsError(true);
      // Reset the error state after the animation plays so she can try again
  ...
  return (
    <motion.div 
      // This is the shake animation logic
      ...
      <div className="relative mb-4">
        <input 
          ...
          onChange={(e) => {
            ...
            if (isError) setIsError(false); // Clear error when she starts typing again
          }}
          className={`w-full p-4 bg-rose-50/50 border-2 rounded-2xl focus:outline-none text-center text-lg font-semibold text-slate-800 transition-all ${
            isError ? "border-red-400" : "border-rose-100 focus:border-rose-400"
          }`}
